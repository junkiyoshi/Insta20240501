...
void ofApp::update() {
	...
	this->rotation_list.push_back(glm::vec3(
		ofMap(ofNoise(noise_param.x, ofGetFrameNum() * 0.015), 0, 1, -360, 360),
		ofMap(ofNoise(noise_param.y, ofGetFrameNum() * 0.015), 0, 1, -360, 360),
		ofMap(ofNoise(noise_param.z, ofGetFrameNum() * 0.015), 0, 1, -360, 360)));
	ofColor color(239);
	...
	for (int i = 0; i < this->radius_list.size(); i++) {
		...
		this->setRingToMesh(this->face, this->frame, glm::vec3(), this->rotation_list[i], this->radius_list[i], this->radius_list[i] * 0.1, ofColor(239), ofColor(0));
	}
}
